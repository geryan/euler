...
for(i in 1:20){
  for(j in 1:20){
    ...
    if(any(iz < 1 | iz > 20) | any(jz < 1 | jz > 20)){
      z[i,j] <- NA_real_
    } else {
      z[i,j] <- m[iz[1], jz[1]] * m[iz[2], jz[2]] * m[iz[3], jz[3]] * m[iz[4], jz[4]]
    }

    if(any(iy < 1 | iy > 20) | any(jy < 1 | jy > 20)){
      y[i,j] <- NA_real_
    } else {
      y[i,j] <- m[iy[1], jy[1]] * m[iy[2], jy[2]] * m[iy[3], jy[3]] * m[iy[4], jy[4]]
    }
...
max_row_prod <- row_prod[which.max(row_prod)]
max_col_prod <- col_prod[which.max(col_prod)]
max_diag_fwd_prod <- diag_fwd_prod[which.max(diag_fwd_prod)]
max_diag_back_prod <- diag_back_prod[which.max(diag_back_prod)]
